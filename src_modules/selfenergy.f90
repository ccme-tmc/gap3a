...
! !PUBLIC VARIABLES:
      ...
      integer:: iop_dvh=0        ! control whether considering the change of Hartree potential during the self-consistency
                                 ...
                                 !  1 -- yes
      integer:: n_sigm=1
      integer:: isym_kbz=1       ! determining whether calculating Sxcmn and Vxcmn in the full (0) or irreducible BZ 
...
! real quantities  
      ...
      real(8),allocatable:: vxcnn(:,:,:)  ! the diagonal elements of vxc
!
...
! sigm* -- full matrix of self-energies
      ...
      complex(8),allocatable:: hgw_wann(:,:,:,:)  ! the effective GW Hamiltonian in the Wannier basis 
...
! Variables related to analytic continuation 
      ...
      integer:: npol_ac        ! number of poles used in the analytical continuation  
      integer:: npar_ac        ! Number of parameters sused in analytical continuuation
...
!
      ...
      if(iop_sxc_sc.eq.0) then  ! full GW
        ...
        nom=0
      else if (iop_sxc_sc.eq.2 .or. iop_sxc_sc.eq.3) then ! cohsex
        nom=1
      endif
      n_sigm = nom + 1
     ...
     &         stat=ierr) 
        ...
        elseif(iop_sxc.eq.2) then !! COHSEX
          allocate(                                                       &
     ...
     &           stat=ierr)
        ...
        elseif(iop_sxc.eq.3) then !! SEX-only 
          allocate(                                                       &
     &           sigc(1,ibgw:nbgw,nirkp,nspin),               &
     &           sigc_q(1,ibgw:nbgw,nirkp,nspin),               &
     &           selfc(ibgw:nbgw,nirkp,nspin),                          &
     &           stat=ierr)
          call errmsg(ierr.ne.0,sname,"Fail to init SX")
        endif

        if(iop_sxc.eq.0.or.iop_sxc.eq.2.or.iop_sxc.eq.3) then 
          sigc_q = 0.d0
          sigc = 0.d0
          selfc = 0.d0 
        endif 
     ...
     &   vxcnn(ibgw:nbgw,nirkp,nspin),                      &
     &   znorm(ibgw:nbgw,nirkp,nspin),                      &
     &   hks_wann(nlmorb,nlmorb,nkp,nspin),                 &
     &   hgw_wann(nlmorb,nlmorb,nkp,nspin),                 &
     &   vxc_wann(nlmorb,nlmorb,nkp,nspin),                 &
     &   sxc_wann(nlmorb,nlmorb,nomeg,nkp,nspin),           &
     &   sacpar(npar_ac,ibgw:nbgw,nirkp,nspin ),            &
     &   stat=ierr)
        call errmsg(ierr.ne.0,sname,"Fail to init sigmx etc")
        sigm = 0.d0
...
!
      ...
      if(iop.eq.0) then 
        deallocate(sigx,sigx_q,selfx,vxcnn) 
        if(iop.eq.0) then 
          deallocate(sigc,sigc_q,sacpar,znorm,selfc)
        elseif(iop.eq.2.or.iop.eq.3) then 
          deallocate(sigc,sigc_q,selfc)
        endif
     ...
     &           sxcmn,dvmn_old,egk)
      ...
      elseif(iop.eq.2) then 
        deallocate(sigm,qpwf_coef,qpwf_dmat,vxcmn,sigx,sigc,selfx,selfc,&
     &   vxcnn,znorm,hks_wann,hgw_wann,vxc_wann,sxc_wann)  
