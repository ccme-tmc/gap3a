!BOP
!
! !MODULE: lebedev_laikov
      module lebedev_laikov
      
! !PUBLIC VARIABLES:
      
      real(8), allocatable :: xleb(:) ! xleb-coordinate of the grid point ileb.
      real(8), allocatable :: yleb(:) ! yleb-coordinate of the grid point ileb.
      real(8), allocatable :: zleb(:) ! zleb-coordinate of the grid point ileb.
      real(8), allocatable :: wleb(:) ! weight of the grid point ileb.

      integer(4) :: nleb ! number of grip points in the sphere.
      real(8), private :: a,b,v
      contains
!EOP      

!BOP
!
! !ROUTINE: unset_lebedev_laikov_grid
!
! !INTERFACE:
      subroutine unset_lebedev_laikov_grid

! !DESCRIPTION:
!
! Write a short description of the subroutine in latex format
!
!EOP
!BOC
      if(allocated(xleb))deallocate(xleb)
      if(allocated(yleb))deallocate(yleb)
      if(allocated(zleb))deallocate(zleb)
      if(allocated(wleb))deallocate(wleb)
      
      end subroutine unset_lebedev_laikov_grid
!EOC
      
!BOP
!
! !ROUTINE: set_lebedev_laikov_grid
!
! !INTERFACE:
      subroutine set_lebedev_laikov_grid(ngrid)
...
!      nleb = ngrid
      ...
      grid_exist = allocated(xleb) .or. allocated(yleb) .or.           &
    &              allocated(zleb) .or. allocated(wleb)
    
      if(grid_exist)then
        write(6,*)'Warning: lebedev_laikov grid already exists'
        write(6,*)'I will unset it!!'
        write(6,*)'Check that the code calls unset_lebedev_laikov_grid'
        write(6,*)'between succesive calls to set_lebedev_laikov_grid'
...
grid: select case (ngrid)
     ...
     & Lebenev integration is greater than 5811'
      ...
      end subroutine set_lebedev_laikov_grid
!EOC      
...
! !DESCRIPTION:
!
!\underline{Lebedev grids of orders nleb=6m+5 where m=0,1,...,21 in 16 digit
!precision}
!
!
!The file Lebedev-Laikov.F implements a set of subroutines providing 
!Lebedev-Laikov grids of order nleb=2m+1, where m=1,2,...,15, and additionally
!grids of order nleb=6m+5, where m=5,6,...,21. The parameters ensure 
!that angular integration of polynomials $xleb^k yleb^l zleb^m$, where $k+l+m \le
!131$ 
!can be performed with a relative accuracy of $2^{-14}$ [1]. Note that the weights
!are normalised to add up to 1.0.
!
!For each order nleb a separate subroutine is provided named 
!LD. The parameters xleb, yleb, zleb are arrays for the 
!cartesian components of each point, and the parameter wleb is an array for the
!!weights. The subroutines increase the integer(4) :: parameter nleb by number of grid
...
!   C-code [1] was kindly provided by Dr. Dmitri nleb. Laikov and 
...
!
!\begin{enumerate}
!   \item V.I. Lebedev, and D.nleb. Laikov
...
!    weight v.
!    The variable nleb is increased by the number of different points
!    generated.
!
