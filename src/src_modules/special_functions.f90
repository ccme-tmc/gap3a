...
subroutine chgm_mod( a, b, x, div, hg, thres, l_rm_erf)

!*****************************************************************************80
!
!! CHGM computes the confluent hypergeometric function M(a,b,x). 
!! This is a modification of the original code, in order to avoid numerical
!! instablity in computing the 2D lattice sum.
!
!  Licensing:
!
...
!    Input, real ( kind = 8 ) X, the argument.
!
!    Input, integer ( kind = 4 ) DIV, the results will be scaled by abs(x)^div.
!
!    Input, real ( kind = 8 ) THRES, threshold for x to use asympotic comput.
!
!    Input, logical L_RM_ERF, remove error function part in asympotic comput.
!
...
!
  ...
  ! For (l,m)=(8,4), (0,0), no following conditions is satisfied
  ...
  ! use Kummer's transformation, always compute x>0
  if ( x < 0.0D+00 ) then
    a = b - a
    a0 = a
    x = abs ( x )
  else
    write(*, "(A40)") "Negative argument x is required. Stop now"
    stop
  end if
  ...
  do n = 0, nl
    ...
    if ( x <= thres .or. a < 0.0D+00 ) then
      ...
      hg1 = tb / tba * x ** ( - a ) * cos ( pi * a ) * sum1 * exp (x0) * x ** div
      ! Note!!! hg2 seems to be related to the error function part,
      ! since this part has no exp(x) term. But is not proved rigorously
      if (l_rm_erf)  then
        hg2 = 0.0D0
      else
        hg2 = tb / ta * x ** ( a - b ) * sum2 * x ** div
      endif
      hg = hg1 + hg2
