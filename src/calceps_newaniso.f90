!BOP
!
! !ROUTINE: calceps_newaniso
!
! !INTERFACE:
      subroutine calceps_newaniso(iq,iom_f,iom_l,isym,isc,iop,lread)
...
! !LOCAL VARIABLES:
      ...
      real(8) :: edif,aux,ffact,omgsq,coefcoul
      ...
      character(len=15)::sname='calceps_newaniso'
...
!
      ...
      if(iq.eq.1.and.iop_coul_c.eq.-1) then
        ...
        write(*,*) "before calchead"
        call calchead_newaniso(1,nirkp,iom_f,iom_l)
      endif
        
      ! set the parallelization over k (within the row)
      call mpi_set_range(nproc_row,myrank_row,nktot,1,ik_f,ik_l)
      write(fid_outgw,*) "calceps_newaniso: myrank_row,ik_f,ik_l =",myrank_row,ik_f,ik_l
...
!        call end_aniso
 ...
 14   format(i4,f16.6,2e16.6)
     ...
     &          czero,epsw2(:,iom),1,ierr)
            call cpu_time(time3)
            call calc_h_w_inv_ang_grid_oo(aniten, iom)
            call proj_head_on_ylm_oo(aniten, iom)
            call cpu_time(time4)
            time_aniso = time_aniso + time4 - time3
          else
          ...
          if(iop_aniso.ne.-1)then
            call angint_eps_sph_oo(aniten, iom, eps(:,:,iom), .FALSE.)
          endif
      ...
      end subroutine calceps_newaniso
!EOC
