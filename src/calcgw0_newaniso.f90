!BOP
!
! !ROUTINE: calcgw0_newaniso
!
! !INTERFACE:
      subroutine calcgw0_newaniso(isxc,iref)
...
! !USES:
     ...
     &                       flag_sxc,fn_selfe,isxc 
      ...
      use anisotropy,  only: init_aniso_oo,end_aniso_oo,iop_aniso,aniten,&
...
!     variables used for parallelization
      ...
      character(len=20):: sname="calcgw0_newaniso"
...
!-----------------------------------------------------------------------                            
        ...
        if(ierr_sc.eq.0) then
          ...
          if(iop_aniso.ne.-1)then
            ...
            call init_aniso_oo(aniten,iq,matsiz,1,nomeg,lmax_gamma,n_ang_grid)
            write(*,*) associated(aniten)
            call cpu_time(time2)
          ...
          if (iop_coul_c.eq.2)then
            ...
            call calceps_newaniso(iq,1,nomeg,0,-1,2,lread_eps)
          else
            call calceps_newaniso(iq,1,nomeg,0,-1,2,lread_eps)
          endif
...
!----------------------------------------------------------------------
      ...
      end subroutine calcgw0_newaniso
!EOC      
