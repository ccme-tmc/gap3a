...
C ifSOflag = table of correspondance isort -> optionSO (1 or 0). Only used for isort with correlated orbitals
        READ(iuinp,*) e_bot,e_top, l_wholeband 
C e_bot, e_top : lower and upper limits of the energy window (in eV)
...
C Printing the size of the Energy window
     ...
     &     e_bot/rev,' eV  to  ',e_top/rev,' eV around the Fermi level.'
        CALL printout(1)
C
C =======================================================================================
...
C Test for checking the impact of the orthonormalization, especially
C when we use the option correlated irreducible representation
C        CALL set_projections(e_bot,e_top)
C ----------------------------------------
C Diagonalization of the correlated occupation matrix obtained with theta projectors
C ----------------------------------------
C        CALL newbasis(.true.)
...
C Setting up the projections for all bands
C ----------------------------------------
C         IF(jobname.eq.'Si') THEN
C           WRITE(buf,'(a)')JOBNAME
C           CALL printout(0)
C           CALL set_projections_silicon(e_bot,e_top,.TRUE.)
C           CALL set_projections_silicon(e_bot,e_top,.FALSE.)
C         ELSE 
...
C Diagonalization of the density matrix
C         CALL newbasis(.true.)
...
C ------------------------------------------------ 
         IF(.NOT.ifBAND) THEN
          ...
          CALL outcrpa(elecn,qtot,e_bot,e_top)
         ELSE
          CALL outband
         ENDIF
         CALL outbwin
