        SUBROUTINE set_projections_silicon(e1,e2,silicon)
C %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
C %%                                                                 %%
...
C
        ...
        CALL printout(0)
c BANDNUMMAX for SILICON
        IF (silicon) then
          BANDNUMMAX=8
          WRITE(buf,'(a)')'Adaptation for the SP3 of SILICON'
          CALL printout(0)
          WRITE(buf,'(a,i5)')'BANDNUMMAX',BANDNUMMAX
          CALL printout(1)
        ENDIF
        OPEN(189,file='Si-bandnum',status='UNKNOWN')
...
C The loop on the band ib is stopped, since all the bands after ib have an energy > that of ib. 
              ELSEIF(silicon.AND.bandnum.GE.BANDNUMMAX) then
C Adaptation for the case of Silicon to describe sp3 states by Wannier states. We need to fix the number of bands inside the window at BANDNUMMAX 
                 kp(ik,is)%nb_top=ib-1
                 kp(ik,is)%included=.TRUE.
                 write(189,*)'BEFORE EXIT',kp(ik,is)%nb_bot,
     &                 kp(ik,is)%nb_top,bandnum
                 EXIT
...
C then kp%included remains at the value .FALSE. and the field kp%nb_top and kp%nb_bot are set to 0.
	    ...
	    ENDIF
            WRITE(189,*)'SUMMARY'
            WRITE(189,*)ik,kp(ik,is)%nb_bot,kp(ik,is)%nb_top
