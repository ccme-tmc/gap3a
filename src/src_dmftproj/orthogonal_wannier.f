...
C ----------------------------
        ...
        COMPLEX(KIND=8), DIMENSION(:,:), ALLOCATABLE :: Dmat_orth
...
C Creation of Dmat :
C ------------------
        ALLOCATE(Dmat(1:ndim,1:ndim),Dmat_orth(1:ndim,1:ndim))
C 
C =====================================================================
...
C -------------------------------------------
            IF(ik.eq.1) then
         WRITE(oudmatov,'(a)')'Overlap matrix (ndim*ndim) betw. Wannier'
         WRITE(oudmatov,'(a,2x,i4)')'ndim',ndim
            ENDIF
            WRITE(oudmatov,'(a)')
            WRITE(oudmatov,'(2(a,2x,i4,2x))')'is=',is,'ik=',ik
            WRITE(oudmatov,'(a)')
            WRITE(oudmatov,'(a)')'Before orthonormalization'
            DO ii=1,ndim
               WRITE(oudmatov,'(50(f15.8,2x))') (Dmat(ii,jj),jj=1,ndim)
            ENDDO
...
C Computation of Dmat_orth: overlap matrix after orthonormalization
            CALL ZGEMM('N','C',ndim,ndim,nbnd,DCMPLX(1.D0,0.D0),
     &        D_orth,ndim,D_orth,ndim,DCMPLX(0.D0,0.D0),Dmat_orth,ndim)
            WRITE(oudmatov,'(a)')'After orthonormalization'
            DO ii=1,ndim
           WRITE(oudmatov,'(50(f15.8,2x))') (Dmat_orth(ii,jj),jj=1,ndim)
            ENDDO
C
C --------------------------------------------------------------------------------
