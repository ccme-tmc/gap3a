#!/usr/bin/env python
"""python functions used by gap_* scripts
"""
...
def coef_hyb(symbl):
    ...
    if symbl == 'sp3':
        ...
        raise ValueError("not implemented for symbl=", symbl)
    ...
    if l < 0: # composite projects (following the convention in Wannier90, Table 3.2 in wannier90 guide)  
        if l == -3:    # sp3 hybrid 
          ...
          norb = 2 
            ...
            raise ValueError("ill-defined value for negative l=", l)
        ...
        coefs_dict = {
                's': s_real, 'p': p_real, 'd': d_real, 'f': f_real,
                'deg': d_real[0:2], 'dt2g': d_real[2:5],
                }
        coefs = coefs_dict.get(coefs, None)
    ...
    try:
        ...
        if nval == 0:
            ...
            if isinstance(def_val,int):
                val = int(sys.argv[i_op])
            elif isinstance(def_val,float):
                val = float(sys.argv[i_op])
            else:
                val = sys.argv[i_op].strip()
            del sys.argv[i_op]
        else:
            val = []
            for i in range(nval):
                if isinstance(def_val[i],int):
                    ...
                    t = float(sys.argv[i_op])
                else:
                    t = sys.argv[i_op].strip()
                del sys.argv[i_op]
                val.append(t)
    except:
        val = def_val
