...
C then kp%included remains at the value .FALSE. and the field kp%nb_top and kp%nb_bot are set to 0.
	    ...
	    ENDIF
            ...
            nbot_max = max( kp(ik,is)%nb_bot, nbot_max )   !! JH
            ntop_min = min( kp(ik,is)%nb_top, ntop_min )   !! JH
          ...
          do is=1,ns
            do ik=1,nk
              if(kp(ik,is)%included) then 
                kp(ik,is)%nb_bot = nbot_max
                kp(ik,is)%nb_top = ntop_min 
              endif 
            enddo 
          enddo 
        endif 
        ...
        DO icrorb=1,ncrorb
          ...
          IF (l==0) THEN
            ...
            DO ib = kp(1,1)%nb_bot,kp(1,1)%nb_top
              ...
              IF (ifSP) WRITE(18,*) pr_crorb(icrorb,1,2)%mat_rep(:,ib)
              WRITE(18,'()')
            ENDDO
            WRITE(18,'(a,i4)') 'ik = ', nk
            DO ib = kp(nk,1)%nb_bot,kp(nk,1)%nb_top
              WRITE(18,*) pr_crorb(icrorb,nk,1)%mat_rep(:,ib)
              IF (ifSP) WRITE(18,*) pr_crorb(icrorb,nk,2)%mat_rep(:,ib)
              WRITE(18,'()')
            ENDDO
          ELSEIF (reptrans(l,isrt)%ifmixing) THEN
            WRITE(18,'(a,i4)') 'ik = ', 1
          ...
          IF (l==0) THEN
            ...
            DO i=1,n
              ...
              DO ib = kp(1,1)%nb_bot,kp(1,1)%nb_top
                ...
                IF (ifSP) WRITE(18,*) pr_orb(iorb,1,2)%matn_rep(:,ib,i)
                WRITE(18,'()')
              ENDDO
            ENDDO
            WRITE(18,'(a,i4)') 'ik = ', nk
            DO i=1,n
              ...
              DO ib = kp(nk,1)%nb_bot,kp(nk,1)%nb_top
                ...
                IF (ifSP)WRITE(18,*) pr_orb(iorb,nk,2)%matn_rep(:,ib,i)
                WRITE(18,'()')
              ENDDO
            ENDDO
          ELSEIF(reptrans(l,isrt)%ifmixing) THEN
