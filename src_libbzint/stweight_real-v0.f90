...
!BOC
     ...
     &           vdif(4,1)**2*vdif(4,3)
            ...
            if(dabs(cc)*weighttol.ge.dabs(aa))      &
     ...
     &                 deltae_vert,vdif,aa,cc
        endif
      enddo
      if(lwarn) then
        write(6,*) "WARNING: sth. unexpected in libbzint->"//trim(sname)&
     &   //" -- check libbzint output file"
      endif 

    1 format('warning in stweight_real: vertix:',i4,' weightt =',g18.10,&
